...
例如[著名八嘎家上学大人](/market?keyword=email:1919892171@qq.com)，可以在用户id中填写上学的id，关键词中填写“八嘎”，在回复组中填写“八嘎！”“bakabaka！” 即可随机选取一个进行回复。

## 当然你也可以选择所有用户，只需要在用户id项选择填写为*即可，这样关键词回复将会对所有的用户生效。
(慎用)
`;
...
export const Config: Schema<Config> = Schema.object({
  replies: Schema.array(Schema.object({
    id: Schema.union([
      Schema.array(Schema.string()).required().role('table'),
      Schema.const('*').required(),
    ]).required().description('用户ID数组或"*"代表所有用户'),
    keyword: Schema.array(Schema.string()).required().description('关键词 数组').role('table'),
    responses: Schema.array(Schema.string().required().description("回复内容")).required().description('回复内容组(随机挑选)').role('table'),
  })
...
export function apply(ctx: Context, config: Config) {
  ctx.on('message',(session) => {
    ...
    for (const reply of config.replies) {
      ...
      if (isForAllUsers && keywordMatched&&session.userId!=session.selfId) {
        // 从对应的回复中随机选择一个发送
